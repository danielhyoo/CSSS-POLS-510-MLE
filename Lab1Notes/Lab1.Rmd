...
* When this course is over, you should be able to do the following (and more) using $\textsf{R}$:
    + Fit a logistic regression model using both $\textsf{glm}$ and "by hand" using $\textsf{optim}$, extract parameters of interest, and convert the estimates so they can be interpreted in terms of probabilities
    ...
    + Use cross-validation to assess the predictive power of several models and compare these models across a variety of in-sample goodness of fit tests
...
setwd("/Users/danielyoo/CSSS-POLS-510-MLE/Lab1")
