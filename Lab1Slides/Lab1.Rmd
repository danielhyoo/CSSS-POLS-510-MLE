...
1. **Lab Sessions**: Fri, 3:30-5:20pm in Smith 105
    ...
    + Materials will be available on the [**course website**](http://faculty.washington.edu/cadolph/index.php?page=21) and my [**Github**](https://github.com/danielhyoo) site on Wednesday evening
    $\newline$
...
1. When this course is over, you should be able to do the following (and much more) in $\textsf{R}$:
    + Fit a logistic regression model using both the $\textsf{glm}$ function and "by hand" using $\textsf{optim}$, extract parameters of interest, and convert estimates so they can be interpreted in probabilities
    + Compute predicted probabilities and use simulation to find the expected values and confidence intervals of $\hat{\pi}$ across counterfactuals values of $\boldsymbol{x}$ 
    + Use cross-validation to assess the predictive accuracy of several models and compare these models across a variety of in-sample goodness of fit tests
    + Fit a variety of bounded and unbounded count models that account for overdispersion 
