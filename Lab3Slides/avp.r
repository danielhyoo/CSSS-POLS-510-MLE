avp <- function(ya,
                ...
                cex.yax=.75,
                cex.axis=.75,
                output = list(outfile=NULL,high=4.5,wide=4),
  ...
  output.def <- list(outfile=NULL,high=6,wide=7,epsi=FALSE)
  ...
  leg <- union.asym.list(leg,leg.def)
    ...
    if (1-addtoplot) {
      ...
      if (1-is.null(output$outfile)) {
        pdf(output$outfile,pointsize = 14,width=output$wide,height=output$high)
      }
