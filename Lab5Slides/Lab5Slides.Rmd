...
$$\text{Pr}(y_{i}=j|\boldsymbol{x}_{i})=\int^{\tau_{j}}_{\tau_{j-1}}\text{Normal}(\boldsymbol{x_i\beta}, 1)\text{dx}_{i}\boldsymbol{\beta}$$ 
...
Instead, we refer back to the latent variable framework, which produces a similar relationship between our covariates and $y^*$ but now we have more than two categories. $\newline$
...
\text{Pr}(y_i=m|\boldsymbol{x_i})&=F(\tau_{m}-\boldsymbol{x_i\beta})-F(\tau_{m-1}-\boldsymbol{x_i\beta})
\end{split}
\end{equation*}
...
$$\mathcal{L}(\boldsymbol{\beta,\tau|y})=\prod^{n}_{i=1}\Bigg\{\prod^{m}_{j=1}[\Phi(\tau_j|\boldsymbol{x_i\beta}],1)-\Phi(\tau_{j-1}|\boldsymbol{x_i\beta},1)]^{y_{ij}} \Bigg\}$$
$$\mathcal{L}(\boldsymbol{\beta, \tau|y})=\sum^n_{i=i}\sum^m_{j=1}y_{ij}\text{log}[\Phi(\tau_j|\boldsymbol{x_i\beta},1)-\Phi(\tau_{j-1}|\boldsymbol{x_i\beta},1)]$$
